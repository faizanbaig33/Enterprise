...
$buildnumber = Get-Content -Path drop/build.txt
echo $buildnumber

Set-Location enterprise-deployment/manifests
$newPat = "andersenwindows-xm-cm:"+$buildnumber
if($sitecore_environment -eq "dev") {
	Write-Host "Updating Dev manifest for deployment!!"
	Perform-VariableSubstitution -configFilePath "cm.yaml" -pattern 'andersenwindows-xm-cm:latest' -newPattern $newPat

	kubectl delete -f cm.yaml
	kubectl apply -f cm.yaml
}
elseif($sitecore_environment -eq "stage") {
	Write-Host "Updating Stage manifest for deployment!!"
	Perform-VariableSubstitution -configFilePath "cm-uat.yaml" -pattern 'andersenwindows-xm-cm:latest' -newPattern $newPat
